...
La vitamina C es una molécula hidrosoluble, es decir, que se disuelve fácilmente en agua y puede ser excretada con facilidad por la orina[^5]. El cuerpo no es capaz de producir ni almacenarla por su cuenta, por lo que es fundamental su consumo a través de la dieta[^2][^3][^4]. Las fuentes principales son frutas y verduras sin cocer, ya que la cocción puede disminuir su contenido; la exposición al sol también puede alterar su estabilidad y concentración[^1].  
...
La absorción ocurre principalmente en la sección final del intestino delgado llamado íleon y se considera que en menor medida podría ser absorbida en secciones anteriores del intestino delgado, esto porque en pH más bajos se presenta de forma no ionizada permitiéndose pasar por las paredes del intestino por difusión pasiva[^5][^7]. La concentración máxima en plasma se obtiene 2- 4 horas tras su consumo[^12][^11][^10][^9]. 
...
Un factor importante en su absorción y transporte es la proteína transportadora sodio-dependiente de vitamina C (SVCT1), y como su nombre indica, esta es dependiente de la concentración de sodio[^5]. Este es un canal saturable por lo que su absorción no es linear, es decir que a mayor dosis no necesariamente habrá una mayor absorción, por ejemplo, con 200 mg se absorbe un 100% del contenido, con 500 mg un 75% y con 1 gramo baja aproximadamente un 50%[^13]. 
...
La distribución la vitamina C en el cuerpo es muy diferenciada, órganos importantes como el corazón, cerebro, pulmones, musculo, intestino, riñones e hígado mantienen un equilibrio homeostático de la vitamina C, en momentos de escasez de la vitamina en el cuerpo estos obtienen reservas a expensas de otros órganos[^14]. Este equilibrio homeostático es de particular importancia para proteger el cerebro en del daño oxidativo, por lo general este mantiene altas concentraciones, pero en casos de deficiencia es el primero en quitarle vitamina C a otros órganos para protegerse a sí mismo[^15][^16][^17].  
...
Su eliminación se da principalmente de forma renal, solo el 20% del total eliminado lo hace de forma inalterada, el resto se reparte en distintos metabolitos cuya mayor parte es en forma de oxalato[^18]. Lo destacable de este metabolito es que, en aquellos pacientes con tendencia a formar cálculos renales, el exceso de vitamina C puede promover la formación de cálculos, aunque las posibilidades son bajas y esta formación debería iniciarse en altas dosis de vitamina C constantemente por un tiempo prolongado[^19][^1]. 
...
El mecanismo de reabsorción a nivel de los riñones para la vitamina C es saturable. Para dosis de 30-60 mg la excreción es mínima y el 100% de la vitamina C es reabsorbida, pero a concentraciones de 100 mg para arriba ya se ve una excreción aumentada del 25% en adelante[^13][^20].
...
En el cuerpo humano la vitamina C cumple múltiples funciones de importancia, gran parte de estas se deben a su rol como antioxidante[^21] participando en la síntesis de colágeno, síntesis de hormonas como adrenalina y noradrenalina, síntesis de carnitina, en la transcripción genética, protección ante especies reactivas del oxígeno, reducción de hierro en el tracto gastrointestinal y regulación de mecanismos como la desmetilación del ADN, hidrolizacion de factores de transcripción entre otros[^1].  

[^21]: Padayatty, S. J., & Levine, M. (2016). Vitamin C: the known and the unknown and Goldilocks. Oral diseases, 22(6), 463-493.

La accion conjunta de la vitamina C con vitamina E es por su capacidad de reciclar este último. La vitamina E, más específicamente en su forma como alta-tocoferol, es un potente antioxidante, pero este puede convertirse en un prooxidante una vez realizada su función, por lo que la vitamina C se encarga de reducirlo para mantenerlo en su forma útil[^1][^22]. 

[^22]: Carr, A. C., & Maggini, S. (2017). Vitamin C and immune function. Nutrients, 9(11), 1211.

## Advertencias 

Una dosis única de 5 a 10 mg puede producir diarrea y/o dolores abdominales con hinchazón, pero en la mayoría de las personas incluso estas dosis altas pueden ser inofensivas. Consumirlo con comidas puede reducir estos efectos adversos[^23][^24][^25][^26]. 
[^23]:Fukushima, R., & Yamazaki, E. (2010). Vitamin C requirement in surgical patients. Current Opinion in Clinical Nutrition & Metabolic Care, 13(6), 669-676.
...
Se ha reportado casos de hemolisis en pacientes con deficiencia de la enzima glucosa-6-fosfato deshidrogenasa, en caso de sospecharse hemolisis por exámenes de sangre suspender la suplementación con vitamina C[^27]. 
...
Algunos exámenes de laboratorio comunes utilizan por medio reacciones de oxido- reducción, por ejemplo, el conteo de leucocitos, para comprobar los niveles de nitrito y bilirrubina, test de sangre, orina, entre otros[^28]. Por lo que en el caso de realizarse estos exámenes se recomienda suspender la suplementación con vitamina C por lo menos 24 horas antes del examen. 
...
En aquellos pacientes con historial de cálculo renal o estén cursando con un deterioro renal se deben evaluar por exámenes renales la posibilidad de una nefropatía por oxalato[^19][^1]. 

## Accion protectora sobre la función endotelial 

La función endotelial es un foco central en la ateroesclerosis. 

Se ha verificado que altos niveles de vitamina C en plasma se asocian con mayores niveles de BH4 (Tetrahidrobiopterina)[^29]. Esto puede deberse a una accion protectora de la vitamina C que sostiene sobre la BH4. 
...
La relación de la concentración de vitamina C respecto la progresión de la enfermedad y sobre la prevención de esta es controversial, aunque está bien documentado que en pacientes con diabetes tienen menor concentración de vitamina C en plasma[^42][^43][^44][^45]. Esto se puede deber a múltiples factores como una mala alimentación de base en los pacientes, una menor biodisponibilidad desde la dieta, mayor excreción en la orina o por mayores requerimientos por su accion antioxidante[^39][^40][^41]. Se ha concluido que la menor concentración de vitamina C es más una consecuencia de la enfermedad que una causante de esta[^33]. 
...
En recientes metaanálisis se observado una disminución estadísticamente importante de la concentración de glucosa post pandrial tras la suplementación con vitamina C[^46]. Otro estudio de regresión se encontró que por cada incremento del 1% de HbA1c en un paciente con diabetes, la suplementación con vitamina C lo disminuía un 0,47%. En general, por cada día que se suplementaba con vitamina C se disminuía un 0.0006% el marcador HbA1c y la glucosa en ayunas disminuía 0,005 mmol/L[^46]. 
